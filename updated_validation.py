...
def all_patients():
    ...
    patient_dict = patients.to_dict(orient='records')
    # Pass data to the template
    return render_template('results.html', patients=patient_dict)
...
def cluster():
    try:
        ...
        # Prepare the new patient data for clustering
        ...
        hospital_patients = pd.read_csv('oncosupport_dataset.csv')
        hospital_patients = pd.concat([hospital_patients, new_patient], ignore_index=True)
        # Save the updated data back to the CSV file
        hospital_patients.to_csv('oncosupport_dataset.csv', index=False)
        # Define categorical and numerical columns
        categorical_cols = ['Gender', 'Ethnicity', 'Diagnosis by Location', 'Cancer Diagnosis', 'Primary Language', 'Parenting Situation', 'Personal Concern']
